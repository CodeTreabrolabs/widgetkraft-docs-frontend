...
const HTML_CODE = `<!-- 1. Add this where you want the chatbot to appear -->
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.ChatbotWidget) {
        window.ChatbotWidget.init({ widgetId: 'YOUR_WIDGET_ID' });
      }
    };
...
const NEXTJS_CODE = `// components/ChatbotWidget.js
'use client';
...
const descMap = {
  ...
  react:  'Drop this into your root component the chatbot loads automatically on mount.',
  nextjs: 'Create a Client Component and add it to your root layout for site-wide coverage.',
};

// Minimal override so the highlighter background stays transparent
// (the outer container provides the bg-gray-800 background)
const syntaxStyle = {
  ...vscDarkPlus,
  'pre[class*="language-"]': {
      ...
      <div className="docs-code-panel">
        ...
        <div className="px-5 py-3 border-t border-gray-700 bg-gray-900/40 flex items-center gap-2">
          ...
          <span className="text-[var(--docs-stone)] text-xs">
            ...
            <strong className="text-[var(--docs-steel)]">Code</strong> tab of your chatbot editor.
          </span>
        </div>
      </div>
    </section>
