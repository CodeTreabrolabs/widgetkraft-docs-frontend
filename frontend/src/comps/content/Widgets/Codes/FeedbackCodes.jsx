...
const HTML_CODE = `<!-- 1. Add this where you want the feedback widget to appear -->
...
export default function App() {
  useEffect(() => {
    ...
    script.src =
      'https://unpkg.com/@getwidgets/feedback-loop-widget/dist/feedback-loop-widget.umd.js';

    script.onload = () => {
      if (window.FeedbackLoopWidget) {
        window.FeedbackLoopWidget.init({ widgetId: 'YOUR_WIDGET_ID' });
      }
    };
  ...
  return (
    <div>
      {/* Your app content */}
      <div id="feedbackloop-root" />
    </div>
  );
}`;

const NEXTJS_CODE = `// components/FeedbackLoopWidget.js
'use client';

import { useEffect } from 'react';

export default function FeedbackLoopWidget() {
  useEffect(() => {
    const script = document.createElement('script');
    script.src =
      'https://unpkg.com/@getwidgets/feedback-loop-widget/dist/feedback-loop-widget.umd.js';
  ...
  return <div id="feedbackloop-root" />;
}

// app/layout.js  (or on a specific page)
import FeedbackLoopWidget from '@/components/FeedbackLoopWidget';
...
const descMap = {
  ...
  react:  'Drop this into your root component the feedback widget loads automatically on mount.',
      ...
      <div className="bg-gray-800 border border-t-0 border-gray-700 rounded-b-lg rounded-tr-lg overflow-hidden">
        ...
        <div className="px-5 py-3 border-t border-gray-700 bg-gray-900/40 flex items-center gap-2">
          ...
          <span className="text-gray-400 text-xs">
            ...
            <strong className="text-gray-300">Code</strong> tab of your feedback widget editor.
          </span>
        </div>
      </div>
    </section>
